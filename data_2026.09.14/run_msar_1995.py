"""Pooled vs Normal RE, common Q dummies, sample from 1995Q1."""
...
OUT_PDF = HERE / "msar_1995.pdf"
TEX = HERE / "msar_1995.tex"
FIGS = HERE / "figs"
...
def _tex_report(sample_line, fitted):
    parts = [
        ...
        r"\title{Panel MS-AR(1) with quarterly dummies, 1995Q1 onward\\[0.4em]"
        ...
        r"\end{align}",
        r"The sample is restricted to calendar time from 1995Q1. "
        r"Specification 1 sets $a_i\equiv a$. Specification 2 lets "
        r"$a_i\sim\mathcal{N}(\alpha,\omega^2)$, integrating each country "
        r"by 11-point Gauss--Hermite. The slope $g$ and the quarterly dummies "
        r"are common. $|\rho|<0.99$. Median $\mu$ pinned at 0. "
        r"Cycles use posterior-mean $a_i$ in the RE specification. "
...
def main():
    df = load_panel()
    df = df.loc[df["time"] >= TMIN].copy()
    sample_line = (
        rf"2026-09-14 panel from 1995Q1: log real GDP per worker. "
