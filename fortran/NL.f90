...
! ---------
!
real(wp), intent(in):: a(:)   ! array of numbers
...
do i = 1, N-1
  ...
  SUBROUTINE discretizeAR1(mmean, rrho, sstd, nn, nsds, ggrid, ttran, stationary)
    ...
    IF (ABS(rrho) >= 1.0_wp) THEN
      ERROR STOP 'discretizeAR1: |rho| must be < 1'
    END IF
    ...
    CALL tauchenTransitions(ggrid, mmean, rrho, sstd, ttran)
    CALL markovStationary(ttran, stationary)
  ...
  !
  !
  SUBROUTINE mesh2Markovs(grid1, tran1, grid2, tran2, bigGrid1, bigGrid2, bigTran, mmap, revmap, stt)
    ...
    REAL(wp), INTENT(OUT), DIMENSION(SIZE(grid1) * SIZE(grid2)) :: stt
    ...
    CALL markovStationary(bigTran, stt)
  ...
  !
  !
  SUBROUTINE tauchenTransitions(ggrid, mmean, rrho, sstd, ttran)
    REAL(wp), INTENT(IN), DIMENSION(:) :: ggrid
    REAL(wp), INTENT(IN) :: mmean, rrho, sstd
    REAL(wp), INTENT(OUT), DIMENSION(SIZE(ggrid), SIZE(ggrid)) :: ttran
    INTEGER :: nn, ix, iPx
    REAL(wp) :: tmp1, tmp2, tmp3, tmp4, valWhere, condMean, sd

    nn = SIZE(ggrid)
    sd = MAX(sstd, 1.0E-12_wp)
    IF (nn == 1) THEN
      ttran(1, 1) = 1.0_wp
      RETURN
    END IF
    ...
    DO ix = 1,nn
      ...
      DO iPx = 1,nn
        IF (iPx == 1) THEN
          ...
          CALL cumnor( (valWhere - condMean) / sd, tmp1, tmp2)
          ttran(ix, 1) = tmp1
        ELSEIF (iPx == nn) THEN
        ...
        ELSE
          ...
          CALL cumnor( (valWhere - condMean) / sd, tmp3, tmp4)
          ttran(ix, iPx) = tmp3-tmp1
        END IF
      END DO
      DO iPx = 1,nn
        ttran(ix, iPx) = MAX(ttran(ix, iPx), 0.0_wp)
      END DO
      tmp1 = SUM(ttran(ix, :))
      IF (tmp1 <= 0.0_wp) THEN
        ERROR STOP 'tauchenTransitions: a row sums to 0'
      END IF
      ttran(ix, :) = ttran(ix, :) / tmp1
    END DO
  END SUBROUTINE tauchenTransitions

  !
  !
  !
  SUBROUTINE markovStationary(ttran, stationary)
    REAL(wp), INTENT(IN), DIMENSION(:, :) :: ttran
    REAL(wp), INTENT(OUT), DIMENSION(SIZE(ttran, 1)) :: stationary
    REAL(wp), DIMENSION(SIZE(ttran, 1)) :: stt0
    REAL(wp) :: tmp1, rowSum
    INTEGER :: n, ix, iter
    ...
    stationary = 0.0_wp
    IF (n < 1) RETURN
    IF (SIZE(ttran, 2) /= n) THEN
      ERROR STOP 'markovStationary: transition matrix is not square'
    END IF
    DO ix = 1,n
      rowSum = SUM(ttran(ix, :))
      IF (ABS(rowSum - 1.0_wp) > 1.0E-6_wp) THEN
        ERROR STOP 'markovStationary: a row of P does not sum to 1'
      END IF
    END DO
    stationary(1) = 1.0_wp
    tmp1 = 1.0_wp
    iter = 0
    DO WHILE (tmp1 > 1.0D-9)
      iter = iter + 1
      IF (iter > maxIter) THEN
        ERROR STOP 'markovStationary: failed to converge'
      END IF
      stt0 = MATMUL( stationary, ttran )
      tmp1 = MAXVAL(ABS( stt0 - stationary ))
      stationary = stt0
    END DO
  END SUBROUTINE markovStationary

  !
  !
  !
  SUBROUTINE farmerTodaRow(ggrid, qrow, eMean, eVar, prow)
    REAL(wp), INTENT(IN), DIMENSION(:) :: ggrid, qrow
    REAL(wp), INTENT(IN) :: eMean, eVar
    REAL(wp), INTENT(OUT), DIMENSION(SIZE(ggrid)) :: prow
    INTEGER :: nn, ix, iter
    REAL(wp) :: lam1, lam2, sw, g1, g2, h11, h12, h22, det
    REAL(wp) :: d1, d2, expo, t2bar, qsum
    REAL(wp), DIMENSION(SIZE(ggrid)) :: z1, z2, ww, qq
    REAL(wp), PARAMETER :: qFloor = 1.0E-12_wp
    REAL(wp), PARAMETER :: gTol = 1.0E-10_wp

    nn = SIZE(ggrid)
    IF (SIZE(qrow) /= nn) THEN
      ERROR STOP 'farmerTodaRow: ggrid and qrow size mismatch'
    END IF
    qsum = SUM(qrow)
    IF (nn == 1 .OR. qsum <= 0.0_wp) THEN
      prow = 0.0_wp
      prow(1) = 1.0_wp
      RETURN
    END IF
    ...
    qq = MAX(qrow, 0.0_wp)
    qq = (1.0_wp - qFloor) * qq / SUM(qq) + qFloor / REAL(nn, wp)
    t2bar = eMean * eMean + MAX(eVar, 0.0_wp)
    DO ix = 1,nn
      z1(ix) = ggrid(ix) - eMean
      z2(ix) = ggrid(ix) * ggrid(ix) - t2bar
    END DO
    ...
    DO iter = 1,80
      sw = 0.0_wp
      DO ix = 1,nn
        expo = lam1 * z1(ix) + lam2 * z2(ix)
        expo = MAX(-60.0_wp, MIN(60.0_wp, expo))
        ww(ix) = qq(ix) * EXP(expo)
        sw = sw + ww(ix)
      END DO
      IF (.NOT. (sw > 0.0_wp) .OR. sw /= sw) EXIT

      prow = ww / sw
      g1 = SUM(prow * z1)
      g2 = SUM(prow * z2)
      IF (MAX(ABS(g1), ABS(g2)) < gTol) RETURN

      h11 = SUM(prow * z1 * z1) - g1 * g1
      h12 = SUM(prow * z1 * z2) - g1 * g2
      h22 = SUM(prow * z2 * z2) - g2 * g2
      h11 = h11 + 1.0E-10_wp
      h22 = h22 + 1.0E-10_wp
      det = h11 * h22 - h12 * h12
      IF (ABS(det) < 1.0E-18_wp) EXIT
  ...
  SUBROUTINE discretizeMSAR(mus, rrhos, sstds, Pi, nn, nsds, &
    ...
    INTEGER, INTENT(IN) :: nn
    REAL(wp), INTENT(IN) :: nsds
    REAL(wp), INTENT(OUT), DIMENSION(nn) :: zgrid
    REAL(wp), INTENT(OUT), DIMENSION(SIZE(mus)*nn, SIZE(mus)*nn) :: bigTran
    INTEGER, INTENT(OUT), DIMENSION(SIZE(mus)*nn, 3) :: mmap
    INTEGER, INTENT(OUT), DIMENSION(SIZE(mus), nn) :: revmap
    REAL(wp), INTENT(OUT), DIMENSION(SIZE(mus)*nn) :: stationary
    INTEGER :: nreg, is, isp, iz, izp, iixx
    ...
    IF (SIZE(rrhos) /= nreg .OR. SIZE(sstds) /= nreg) THEN
      ERROR STOP 'discretizeMSAR: mus, rrhos, sstds size mismatch'
    END IF
    IF (SIZE(Pi, 1) /= nreg .OR. SIZE(Pi, 2) /= nreg) THEN
      ERROR STOP 'discretizeMSAR: Pi is not nreg x nreg'
    END IF
    DO is = 1,nreg
      IF (ABS(rrhos(is)) >= 1.0_wp) THEN
        ERROR STOP 'discretizeMSAR: |rho| must be < 1'
      END IF
      rowSum = SUM(Pi(is, :))
      IF (ABS(rowSum - 1.0_wp) > 1.0E-6_wp) THEN
        ERROR STOP 'discretizeMSAR: a row of Pi does not sum to 1'
      END IF
    END DO

    IF (nn == 1) THEN
      zgrid(1) = SUM(mus) / REAL(nreg, wp)
      iixx = 0
      DO is = 1,nreg
        iixx = iixx + 1
        mmap(iixx, :) = (/ iixx, is, 1 /)
      ...
      DO is = 1,nreg
        DO isp = 1,nreg
          bigTran(revmap(is, 1), revmap(isp, 1)) = Pi(is, isp)
        END DO
      END DO
      CALL markovStationary(bigTran, stationary)
      RETURN
    END IF
    ...
    DO is = 1,nreg
      ...
      uncondStd(is) = sdUse / SQRT(1.0_wp - rho2)
    END DO
    half = nsds * MAXVAL(uncondStd)
    CALL linspace(zgrid, MINVAL(mus) - half, MAXVAL(mus) + half, nn)

    iixx = 0
    DO is = 1,nreg
      DO iz = 1,nn
        iixx = iixx + 1
        mmap(iixx, :) = (/ iixx, is, iz /)
        revmap(is, iz) = iixx
      END DO
    END DO
  ...
  ! Linear interpolation on a uniform grid xx. Same endpoint clamp as linInterp.
  PURE FUNCTION linInterpUniform(xx, yy, xval) RESULT(yval)
    REAL(wp) :: yval
    REAL(wp), DIMENSION(:), INTENT(IN) :: xx, yy
    REAL(wp), INTENT(IN) :: xval
    INTEGER :: nn, low
    ...
    x0 = xx(1)
    dx = (xx(nn) - x0) / REAL(nn - 1, wp)
    pos = (xval - x0) / dx
    low = 1 + FLOOR(pos)
    IF (low < 1) low = 1
    IF (low > nn - 1) low = nn - 1
    yval = yy(low) + (yy(low + 1) - yy(low)) * (pos - REAL(low - 1, wp))
  END FUNCTION linInterpUniform
