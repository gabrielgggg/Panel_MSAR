...
With `random_intercepts=True`, \(a\) is replaced by \(a_i\sim N(\alpha,\omega^2)\). With `convergence=True`, a decaying gap is added on top of those intercepts: \(y_{it}=a_i+gt+b_i\lambda^{t-T_{i0}}+z_{it}\) (\(b_i\sim N(0,\omega_b^2)\) independent of \(a_i\); \(T_{i0}\) is country \(i\)'s first observation). \(\lambda\) may be fixed (`lambda_value=0.98` is Barro 2%/year) or free in \((0,\texttt{lambda\_max})\) with default cap 0.985. The slope \(g\) is always common.
...
| Catch-up | Off by default (`convergence=False`). If on, \(y_{it}=a_i + g t + b_i\lambda^{t-T_{i0}}+z_{it}\). |
