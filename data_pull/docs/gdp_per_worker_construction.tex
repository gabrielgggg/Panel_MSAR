...
QNEA publishes both \texttt{SA} and \texttt{NSA}. The choice is made
\emph{at the country level}, never spliced inside a country:

\begin{itemize}
\item If a country has any SA constant-price GDP, all of that country's GDP
observations come from SA (NSA is ignored for that country).
\item Otherwise the country is filled from NSA.
\end{itemize}

The current vintage uses SA for 68 countries and NSA for 33. The choice is
stored in \texttt{gdp\_sa} and in \texttt{metadata}.
...
GDP SA / NSA (country-level) & 68 / 33 \\
